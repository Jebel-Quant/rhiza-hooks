...
`rhiza_hooks` ships seven pre-commit hooks, each implemented as a standalone module with a `main()` entry point.
...
| [`check_readme_version`](check_readme_version.md) | `check-readme-version` | Assert README quick-start `rev:` matches `pyproject.toml` version |
