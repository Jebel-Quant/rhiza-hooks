...
- If README is not updated, ensure it still contains `<!-- MAKE_HELP_START -->
```
[36m  ____  _     _
 |  _ \| |__ (_)______ _
 | |_) | '_ \| |_  / _\`|
...
[1mRhiza Workflows[0m
  ...
  [36m_apply-sync-schedule[0m  (internal) apply RHIZA_SYNC_SCHEDULE override to GitHub Actions sync workflow
  [36mmaterialize         [0m  [DEPRECATED] use 'make sync' instead — materialize --force is now sync
  [36msummarise-sync      [0m  summarise differences created by sync with template repository
  [36mrhiza-test          [0m  run rhiza's own tests (if any)
...
[1mMeta[0m
  ...
  [36mci-os-matrix        [0m  Emit GitHub CI OSes (RHIZA_CI_OS_MATRIX as JSON array, default ["ubuntu-latest"])

[1mBook[0m
  [36mserve               [0m  build and serve the book at http://localhost:8000
  [36mbook                [0m  compile the companion book via MkDocs
...
[1mDev[0m
  [36mdoctor              [0m  verify local prerequisites and print actionable guidance
...
[1mQuality and Formatting[0m
  [36mall                 [0m  run all CI targets locally
  [36mdeptry              [0m  Run deptry
  [36mfmt                 [0m  check the pre-commit hooks and the linting
  [36mtodos               [0m  search and report all TODO/FIXME/HACK comments in the codebase
  [36msuppression-audit   [0m  scan codebase for inline suppressions and report (grade, detail, histogram)
  [36msemgrep             [0m  run Semgrep static analysis
  [36mlicense             [0m  run license compliance scan (fail on GPL, LGPL, AGPL)

[1mReleasing and Versioning[0m
  [36mbump                [0m  bump version of the project (supports DRY_RUN=1)
  [36mrelease             [0m  create tag and push to remote repository triggering release workflow (supports DRY_RUN=1)
  [36mpublish             [0m  bump version, create tag and push in one step (supports DRY_RUN=1)
  [36mrelease-status      [0m  show release workflow status and latest release information

[1mDevelopment and Testing[0m
  [36mtest                [0m  run all tests
  [36mtypecheck           [0m  run ty type checking
  ...
  [36mhypothesis-test     [0m  run property-based tests with Hypothesis
  [36mstress              [0m  run stress/load tests
  [36mmutation            [0m  run mutation tests with mutmut
  [36mtest-pyproject      [0m  run pyproject.toml structure tests

```
<!-- MAKE_HELP_END -->` markers.
